...
class ModelProducto
{
    ...
    function getProductosConCategoria() {
        $query = $this->db->prepare("SELECT *, productos.id as id_producto FROM productos JOIN categoria ON productos.id_categoria = categoria.id");
        $query->execute();
    ...
    function insertaProducto($aroma, $precio, $categoria, $propiedad, $duracion)
    {
        $sentencia = $this->db->prepare(" INSERT INTO productos(aroma,precio,propiedad,duracion,id_categoria) VALUES (?,?,?,?,?)");
        $sentencia->execute([$aroma, $precio,$propiedad, $duracion,$categoria]);
    }
    ...
    function modificarProducto($id, $aroma, $precio, $categoria, $propiedad, $duracion)
    {
        $sentencia = $this->db->prepare(" UPDATE productos SET aroma=?, precio=?, id_categoria=?, propiedad=?, duracion=? WHERE id=?");
        $sentencia->execute([$aroma,$precio,$categoria,$propiedad,$duracion,$id]);
    }
}
